...
process_covars = function(data, cancer, outcome){
  ...
  if(outcome %in% covars){
    drop_outcome = which(colnames(data)==outcome)
    data = data[,-drop_outcome]
  }
  if(outcome %in% c("AJnonAJ", "NWSE")){
    data = subset(data, select = -c(PC1, PC2, PC3, PC4, PC5))
  }
...
for(cancer in all_cancer){
  ...
  if(dim(data[which(data$TMB.H==1),])[1]>=10){
    data[which(data$AJnonAJ>=1e-8),"AJnonAJ"] = 1
    data[which(data$AJnonAJ<1e-8),"AJnonAJ"] = 0
    data$NWSE = scale(data$NWSE)
    data[which(data$AJnonAJ==1),"NWSE"] = NA
    data$AJnonAJ = factor(data$AJnonAJ, levels = c("0", "1"), labels = c("nonAJ", "AJ"))
    ...
    for(line in 1:dim(subset)[1]){
        if(subset[line,"Outcome"] == "SEX" & cancer %in% c("Breast_Carcinoma", "Endometrial_Cancer", "Ovarian_Cancer", "Prostate_Cancer")){
          results = rbind(results, c(subset[line,"Cancer"], cancer, subset[line,"Outcome"], NA, NA, NA, NA))
        } else{
...
age_pancancer = pancancer[which(pancancer$IndVar %in% c("Baseline", "Change", "RatioChange")),]
...
for(i in unique(pancancer$IndVar)){
  ...
  meta = rbind(meta, c("Pan-Cancer", i, z$beta, z$se, z$pval))
}

#Metastatic & AJnonAJ not stable
